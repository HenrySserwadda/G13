            ...
            @foreach($users as $user)
                <tr class="max-w-m">
                    ...
                    <td class="py-2 px-4 border-b border-gray-200 dark:border-gray-700">
                        <form method="POST" action="{{ route('dashboard.systemadmin.approve-retailers',$user->id) }}">
                            @csrf
                            <x-primary-button>Approve</x-primary-button>
                        </form>
                    </td>
                    <td class="py-2 px-4 border-b border-gray-200 dark:border-gray-700">
                        <form method="POST" action="{{ route('dashboard.systemadmin.rejectRetailers',$user->id) }}">
                            @csrf
