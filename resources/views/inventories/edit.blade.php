...
<div class="mb-6">
    <h2 class="text-2xl font-bold text-gray-800">Edit Inventory Entry</h2>
</div>
...
<form action="{{ route('inventories.update', $inventory->id) }}" method="POST" class="space-y-6 bg-white p-6 rounded shadow border">
    @csrf
    @method('PUT')
    ...
    <div>
        ...
        <input type="text" disabled value="{{ $inventory->rawMaterial->name ?? 'N/A' }}" class="mt-1 block w-full bg-gray-100 border border-gray-300 text-gray-700 rounded-md shadow-sm">
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
            ...
            <input type="number" name="on_hand" id="on_hand" min="0" value="{{ old('on_hand', $inventory->on_hand) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500">
        </div>
        <div>
            <label for="on_order" class="block text-sm font-medium text-gray-700">Quantity On Order</label>
            <input type="number" name="on_order" id="on_order" min="0" value="{{ old('on_order', $inventory->on_order) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500">
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
            ...
            <input type="date" name="delivered_on" id="delivered_on" value="{{ old('delivered_on', $inventory->delivered_on) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500">
        </div>
        <div>
            <label for="expected_delivery" class="block text-sm font-medium text-gray-700">Expected Delivery</label>
            <input type="date" name="expected_delivery" id="expected_delivery" value="{{ old('expected_delivery', $inventory->expected_delivery) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500">
        </div>
    </div>
