...
<div class="mb-6">
    <h2 class="text-2xl font-bold text-gray-800 dark:text-white">Add Inventory Entry</h2>
</div>
...
<form action="{{ route('inventories.store') }}" method="POST" class="space-y-6 bg-white dark:bg-gray-800 p-6 rounded shadow border dark:border-gray-700">
    @csrf

    <!-- Raw Material Selection -->
    <div>
        <label for="raw_material_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Raw Material</label>
        <select id="raw_material_id" name="raw_material_id" required class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
            ...
            <input type="number" name="on_order" id="on_order" min="0" value="{{ old('on_order') }}" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500">
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
            ...
            <input type="date" name="delivered_on" id="delivered_on" value="{{ old('delivered_on') }}" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500">
        </div>
        <div>
            <label for="expected_delivery" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Expected Delivery</label>
            <input type="date" name="expected_delivery" id="expected_delivery" value="{{ old('expected_delivery') }}" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500">
        </div>
    </div>
